...
import { createOmniClawCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("omniclaw-ws-", async (workspaceDir) => {
      await withTempDir("omniclaw-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createOmniClawCodingTools({ workspaceDir });
    ...
    await withTempDir("omniclaw-ws-", async (workspaceDir) => {
      await withTempDir("omniclaw-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello omniclaw");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("omniclaw-ws-", async (workspaceDir) => {
      await withTempDir("omniclaw-override-", async (overrideDir) => {
        const tools = createOmniClawCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("omniclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("omniclaw-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "omniclaw-sbx-test",
        ...
        const tools = createOmniClawCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
