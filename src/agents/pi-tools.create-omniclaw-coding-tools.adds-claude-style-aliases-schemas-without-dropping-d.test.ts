...
const defaultTools = createOmniClawCodingTools();

describe("createOmniClawCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "omniclaw-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createOmniClawCodingTools();
    ...
    try {
      ...
      const contents = "Hello from omniclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "omniclaw-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "omniclaw-workspace"),
    ...
    const tools = createOmniClawCodingTools({ sandbox });
