...
/**
 * Cleanup suspended OmniClaw CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: OmniClawConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "omniclaw-cli-images-"));
  const paths: string[] = [];
