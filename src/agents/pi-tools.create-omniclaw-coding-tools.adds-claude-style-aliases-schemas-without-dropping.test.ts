...
import { __testing, createOmniClawCodingTools } from "./pi-tools.js";
...
describe("createOmniClawCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createOmniClawTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOmniClawCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "omniclaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "omniclaw-outside.txt");
