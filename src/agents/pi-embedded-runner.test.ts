...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "omniclaw-embedded-agent-"));
...
const ensureModels = (cfg: OmniClawConfig) => ensureOmniClawModelsJson(cfg, agentDir) as unknown;
