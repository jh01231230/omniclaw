...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "omniclaw.sandbox=1");
  args.push("--label", `omniclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `omniclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `omniclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("omniclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`omniclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`omniclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("omniclaw sandbox recreate --all");
}
