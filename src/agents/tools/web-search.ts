...
type WebSearchConfig = NonNullable<OmniClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OmniClawConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("omniclaw configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.omniclaw.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://omniclaw.ai",
      "X-Title": "OmniClaw Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
