...
function resolveDefaultModelRef(cfg?: OmniClawConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: OmniClawConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: OmniClawConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureOmniClawModelsJson(effectiveCfg, params.agentDir);
