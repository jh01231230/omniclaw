...
import { loadOmniClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.OMNICLAW_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `omniclaw-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "omniclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.OMNICLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadOmniClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.OMNICLAW_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadOmniClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOmniClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
