...
import { discoverOmniClawPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OmniClawPluginDefinition;
  register?: OmniClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OmniClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OmniClawPluginDefinition;
...
export function loadOmniClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOmniClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "omniclaw/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OmniClawPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as OmniClawPluginModule;
    } catch (err) {
