...
const SEARCH_TOOL = "https://docs.omniclaw.ai/mcp.SearchOmniClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.omniclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('omniclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.omniclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('omniclaw docs "your query"')}`);
    }
    return;
  }
