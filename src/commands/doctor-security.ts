...
import type { OmniClawConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OmniClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("omniclaw security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("omniclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("omniclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("omniclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("omniclaw config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
