...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OMNICLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OMNICLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OMNICLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("OMNICLAW_GATEWAY_PASSWORD");
  });
