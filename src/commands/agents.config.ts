...
export function listAgentEntries(cfg: OmniClawConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OmniClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: OmniClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OmniClawConfig): AgentSummary[] {
