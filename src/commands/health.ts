...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.OMNICLAW_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: OmniClawConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.OMNICLAW_DEBUG_HEALTH);
    if (opts.verbose) {
