...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOmniClawAgentDir: vi.fn().mockReturnValue("/tmp/omniclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/omniclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/omniclaw-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOmniClawAgentDir: mocks.resolveOmniClawAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOmniClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/omniclaw-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/omniclaw-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/omniclaw-agent-custom");
