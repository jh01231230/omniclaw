...
export async function updateConfig(
  mutator: (cfg: OmniClawConfig) => OmniClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OmniClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OmniClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("omniclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
