...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOmniClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("omniclaw models auth setup-token")}\` or \`${formatCliCommand("omniclaw configure")}\`.`
          : `Run \`${formatCliCommand("omniclaw configure")}\` or set an API key env var.`;
