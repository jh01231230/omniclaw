...
import { readConfigFileSnapshot, type OmniClawConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: OmniClawConfig, model: string): OmniClawConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("omniclaw plugins install")}\`.`,
    );
  }
