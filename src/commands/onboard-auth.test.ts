...
const requireAgentDir = () => {
  const agentDir = process.env.OMNICLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OMNICLAW_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.OMNICLAW_STATE_DIR;
  const previousAgentDir = process.env.OMNICLAW_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.OMNICLAW_AGENT_DIR;
    } else {
      process.env.OMNICLAW_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.OMNICLAW_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OMNICLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "omniclaw-oauth-"));
    process.env.OMNICLAW_STATE_DIR = tempStateDir;
    process.env.OMNICLAW_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OMNICLAW_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OMNICLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "omniclaw-minimax-"));
    process.env.OMNICLAW_STATE_DIR = tempStateDir;
    process.env.OMNICLAW_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OMNICLAW_AGENT_DIR;
