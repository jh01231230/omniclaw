...
function resolveMode(cfg: OmniClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OmniClaw doctor");

  const root = await resolveOmniClawPackageRoot({
  ...
  let cfg: OmniClawConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("omniclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("omniclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("omniclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("omniclaw doctor --fix")}" to apply changes.`);
  }
