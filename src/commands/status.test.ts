...
beforeAll(() => {
  previousProfile = process.env.OMNICLAW_PROFILE;
  process.env.OMNICLAW_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.OMNICLAW_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/omniclaw-root.js", () => ({
  resolveOmniClawPackageRoot: vi.fn().mockResolvedValue("/tmp/omniclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/omniclaw/pnpm-lock.yaml",
      markerPath: "/tmp/omniclaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("OmniClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("omniclaw status --all") ||
          l.includes("omniclaw --profile isolated status --all") ||
          l.includes("omniclaw status --all") ||
          l.includes("omniclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OMNICLAW_GATEWAY_TOKEN = "abcd1234";
    try {
