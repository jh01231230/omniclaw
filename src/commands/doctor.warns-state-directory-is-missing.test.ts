...
beforeEach(() => {
  ...
  resolveOmniClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OMNICLAW_STATE_DIR;
  originalUpdateInProgress = process.env.OMNICLAW_UPDATE_IN_PROGRESS;
  process.env.OMNICLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "omniclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OMNICLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OMNICLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.OMNICLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveOmniClawPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadOmniClawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/omniclaw-root.js", () => ({
  resolveOmniClawPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "omniclaw-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.OMNICLAW_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OMNICLAW_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/omniclaw.json",
      exists: true,
      raw: "{}",
    ...
    process.env.OMNICLAW_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
