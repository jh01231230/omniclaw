...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OmniClaw status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("omniclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("omniclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.omniclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.omniclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("omniclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("omniclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("omniclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("omniclaw gateway probe")}`);
  }
}
