...
const execHostEnforced = process.env.OMNICLAW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.OMNICLAW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureOmniClawCliOnPath({ pathEnv: process.env.PATH ?? "" });
