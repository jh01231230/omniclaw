...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "OmniClaw is a hobby project and still in beta. Expect sharp edges.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "If you’re not comfortable with basic security and access control, don’t run OmniClaw.",
      ...
      "",
      "Must read: https://docs.omniclaw.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(
  ...
  await prompter.intro(
    "Welcome to OmniClaw setup! This wizard will guide you through configuring your self-hosted AI assistant. You'll set up the gateway, choose an AI model, and optionally connect messaging channels (WhatsApp, Telegram, Slack, Discord, etc.).",
  );
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: OmniClawConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.omniclaw.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("omniclaw doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  let flow: WizardFlow =
    ...
    (await prompter.select({
      ...
      options: [
        {
          value: "quickstart",
          label: "QuickStart — recommended for first-time users",
          hint: "Guided setup with sensible defaults. You can fine-tune later via omniclaw configure.",
        },
        {
          value: "advanced",
          label: "Manual — for custom gateway, network, or auth setup",
          hint: "Full control over port, bind address, Tailscale, and auth options.",
        },
      ],
  ...
  if (snapshot.exists) {
    ...
    const action = await prompter.select({
      ...
      options: [
        { value: "keep", label: "Keep existing values", hint: "Continue with current settings" },
        { value: "modify", label: "Update specific values", hint: "Change only what you need" },
        {
          value: "reset",
          label: "Reset and start fresh",
          hint: "WARNING: Can remove config, credentials, and sessions",
        },
      ],
    });
    ...
    if (action === "reset") {
      ...
      const resetScope = (await prompter.select({
        message: "How much do you want to reset?",
        options: [
          {
            value: "config",
            label: "Config only",
            hint: "Remove ~/.omniclaw/omniclaw.json; keep credentials and sessions",
          },
          {
            value: "config+creds+sessions",
            label: "Config + credentials + sessions",
            hint: "Remove config and all stored auth/session data",
          },
          {
            value: "full",
            label: "Full reset — everything",
            hint: "Remove config, credentials, sessions, and workspace files",
          },
        ],
      })) as ResetScope;
  ...
  const localProbe = await probeGatewayReachable({
    url: localUrl,
    token: baseConfig.gateway?.auth?.token ?? process.env.OMNICLAW_GATEWAY_TOKEN,
    password: baseConfig.gateway?.auth?.password ?? process.env.OMNICLAW_GATEWAY_PASSWORD,
  });
  ...
  // install a tiny `~/.local/bin/omniclaw` wrapper + PATH snippet so `omniclaw ...` works from any directory.
  await ensureOnboardingShellWrapper({ prompter, runtime, binName: "omniclaw" });
