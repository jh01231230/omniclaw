...
export function resolveConfigDir(
  ...
  const override = env.OMNICLAW_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const newDir = path.join(homedir(), ".omniclaw");
  try {
...
// Configuration root; can be overridden via OMNICLAW_STATE_DIR.
