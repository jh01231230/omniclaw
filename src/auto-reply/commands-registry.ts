...
import type { OmniClawConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: OmniClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OmniClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OmniClawConfig),
...
export function getCommandDetection(_cfg?: OmniClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OmniClawConfig) {
