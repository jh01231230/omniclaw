 ...
 * 1. OMNICLAW_ALLOW_PUBLIC_BIND=true environment variable
 * 2. OMNICLAW_PUBLIC_BIND_IP_ALLOWLIST is non-empty (comma-separated IPs/CIDRs)
...
function parsePublicBindIpAllowlist(
  ...
  const raw = (env.OMNICLAW_PUBLIC_BIND_IP_ALLOWLIST ?? "").trim();
...
export function assertPublicBindSafe(options: PublicBindGuardOptions): PublicBindGuardResult {
  ...
  // Check 1: OMNICLAW_ALLOW_PUBLIC_BIND must be explicitly true
  const allowPublic = env.OMNICLAW_ALLOW_PUBLIC_BIND === "true";
  if (!allowPublic) {
    remediations.push("Set OMNICLAW_ALLOW_PUBLIC_BIND=true to explicitly allow public binding");
    ...
    log.warn(
      `public bind guard: refusing to bind to ${bindHost} - OMNICLAW_ALLOW_PUBLIC_BIND not set`,
    );
  ...
  if (!ipAllowlist.ok) {
    remediations.push("Fix OMNICLAW_PUBLIC_BIND_IP_ALLOWLIST (comma-separated IPs/CIDRs)");
    remediations.push('Example: OMNICLAW_PUBLIC_BIND_IP_ALLOWLIST="203.0.113.10,198.51.100.0/24"');
    log.warn(
  ...
  if (ipAllowlist.entries.length === 0) {
    remediations.push(
      "Set OMNICLAW_PUBLIC_BIND_IP_ALLOWLIST=<ip1>,<ip2> to specify allowed client IPs",
    );
    ...
    return {
      allowed: false,
      reason:
        "Public binding requires an IP allowlist (OMNICLAW_PUBLIC_BIND_IP_ALLOWLIST is empty)",
      remediations,
    };
  }
  ...
  if (!hasAuth) {
    ...
    remediations.push("  - Set OMNICLAW_GATEWAY_TOKEN (recommended) or OMNICLAW_GATEWAY_PASSWORD");
...
export function enforcePublicBindGuard(options: PublicBindGuardOptions): void {
  ...
  if (!result.allowed) {
    const message = [
      ...
      "   https://docs.omniclaw.ai/security/public-bind",
      "",
      "═══════════════════════════════════════════════════════════════",
      "",
    ].join("\n");
