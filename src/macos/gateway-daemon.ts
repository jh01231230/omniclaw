...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `omniclaw --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.OMNICLAW_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.OMNICLAW_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[omniclaw] Gateway daemon failed:",
