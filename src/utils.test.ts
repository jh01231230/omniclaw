...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "omniclaw-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "omniclaw-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "omniclaw-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "omniclaw-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.omniclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "omniclaw-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/omniclaw")).toBe(path.resolve(os.homedir(), "omniclaw"));
  });
