...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "omniclaw-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "omniclaw doctor",
        managerScriptArgs(manager, "omniclaw", ["doctor", "--non-interactive"]),
        gitRoot,
        { OMNICLAW_UPDATE_IN_PROGRESS: "1" },
      ),
    );
