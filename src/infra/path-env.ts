...
function candidateBinDirs(opts: EnsureOmniClawPathOpts): string[] {
  ...
  // Bundled macOS app: `omniclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "omniclaw");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/omniclaw` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "omniclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `omniclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOmniClawCliOnPath(opts: EnsureOmniClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OMNICLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.OMNICLAW_PATH_BOOTSTRAPPED = "1";
