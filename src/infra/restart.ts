...
export function triggerOmniClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OMNICLAW_SYSTEMD_UNIT,
        process.env.OMNICLAW_PROFILE,
      );
  ...
  const label =
    process.env.OMNICLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OMNICLAW_PROFILE);
