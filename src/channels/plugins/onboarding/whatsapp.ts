...
function setWhatsAppDmPolicy(cfg: OmniClawConfig, dmPolicy: DmPolicy): OmniClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: OmniClawConfig, allowFrom?: string[]): OmniClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: OmniClawConfig, selfChatMode: boolean): OmniClawConfig {
...
async function detectWhatsAppLinked(cfg: OmniClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so OmniClaw can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for OmniClaw" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("omniclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
