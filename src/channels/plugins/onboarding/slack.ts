...
function setSlackDmPolicy(cfg: OmniClawConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "OmniClaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for OmniClaw`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: OmniClawConfig, allowFrom: string[]): OmniClawConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
