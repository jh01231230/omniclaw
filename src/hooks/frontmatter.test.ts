...
`;
    ...
    expect(parsed.omniclaw.emoji).toBe("💾");
    expect(parsed.omniclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.omniclaw.emoji).toBe("📝");
    expect(parsed.omniclaw.events).toEqual(["command"]);
    expect(parsed.omniclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.omniclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"omniclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"omniclaw": {"events": ["test"]}}');
  });
...
describe("resolveOmniClawMetadata", () => {
  it("extracts omniclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOmniClawMetadata(frontmatter);
  ...
  it("returns undefined when omniclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
      ...
      metadata: JSON.stringify({
        omniclaw: {
          ...
          install: [
            ...
            { id: "npm", kind: "npm", package: "@omniclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@omniclaw/hook");
  });
...
`;
    ...
    const omniclaw = resolveOmniClawMetadata(frontmatter);
    expect(omniclaw).toBeDefined();
    expect(omniclaw?.emoji).toBe("💾");
    expect(omniclaw?.events).toEqual(["command:new"]);
    expect(omniclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(omniclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const omniclaw = resolveOmniClawMetadata(frontmatter);
    expect(omniclaw?.emoji).toBe("disk");
    expect(omniclaw?.events).toEqual(["command:new"]);
  });
});
