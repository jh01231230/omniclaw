...
homepage: https://docs.omniclaw.ai/hooks#command-logger
metadata:
  {
    "omniclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.omniclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.omniclaw/logs/commands.log ~/.omniclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/omniclaw`:
   ```
   /home/username/.omniclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
