...
This directory contains hooks that ship with OmniClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.omniclaw.ai/hooks/soul-evil
...
Hooks can be configured in `~/.omniclaw/omniclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'omniclaw.*gateway' && pnpm omniclaw gateway`
3. Enable the hook: `omniclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.omniclaw.ai/hooks
