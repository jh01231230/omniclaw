import type { BrowserConfig, BrowserProfileConfig, OmniClawConfig } from "../config/config.js";
...
import {
  DEFAULT_OMNICLAW_BROWSER_COLOR,
  DEFAULT_OMNICLAW_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_OMNICLAW_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OMNICLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "omniclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OMNICLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OMNICLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OmniClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for omniclaw/omniclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OmniClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OMNICLAW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_OMNICLAW_BROWSER_PROFILE_NAME]
      ? DEFAULT_OMNICLAW_BROWSER_PROFILE_NAME
      : DEFAULT_BROWSER_DEFAULT_PROFILE_NAME);
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "omniclaw";
