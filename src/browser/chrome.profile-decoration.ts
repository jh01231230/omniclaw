...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".omniclaw-profile-decorated");
}
 ...
 */
export function decorateOmniClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OMNICLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OMNICLAW_BROWSER_COLOR).toUpperCase();
