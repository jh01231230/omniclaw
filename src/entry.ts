...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.OMNICLAW_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.OMNICLAW_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.OMNICLAW_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[omniclaw] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[omniclaw] ${parsed.error}`);
    process.exit(2);
  }
