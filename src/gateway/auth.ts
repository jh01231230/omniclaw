...
function resolveAuthRateLimitConfig(env: NodeJS.ProcessEnv): AuthRateLimitConfig {
  const enabled = env.OMNICLAW_AUTH_RATE_LIMIT !== "0";
  const thresholdRaw = env.OMNICLAW_AUTH_RATE_LIMIT_THRESHOLD;
  const windowRaw = env.OMNICLAW_AUTH_RATE_LIMIT_WINDOW_MS;
  const blockRaw = env.OMNICLAW_AUTH_RATE_LIMIT_BLOCK_MS;
...
export function resolveGatewayAuth(params: {
  ...
  const token =
    authConfig.token ?? env.OMNICLAW_GATEWAY_TOKEN ?? env.CLAWDBOT_GATEWAY_TOKEN ?? undefined;
  const password =
    authConfig.password ??
    env.OMNICLAW_GATEWAY_PASSWORD ??
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or OMNICLAW_GATEWAY_TOKEN)",
    );
  }
