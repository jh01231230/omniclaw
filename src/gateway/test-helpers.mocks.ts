...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `omniclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.OMNICLAW_CONFIG_PATH = path.join(root, "omniclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "omniclaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "omniclaw-gateway-test"),
        ...fileDefaults,
