...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    const allowQueryToken = process.env.OMNICLAW_HOOKS_ALLOW_QUERY_TOKEN === "true";
    ...
    if (fromQuery && !token) {
      ...
      res.end(
        "Hook tokens in query parameters are disabled. " +
          "Use Authorization: Bearer <token> or X-OmniClaw-Token header instead.",
      );
      return true;
    }
...
export function createGatewayHttpServer(opts: {
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      const allowlistRaw = (process.env.OMNICLAW_PUBLIC_BIND_IP_ALLOWLIST ?? "").trim();
      if (allowlistRaw) {
