...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.OMNICLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.OMNICLAW_SKIP_GMAIL_WATCHER;
    process.env.OMNICLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.OMNICLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.OMNICLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OMNICLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
