...
export function resolveHooksConfig(cfg: OmniClawConfig): HooksConfigResolved | null {
...
export function extractHookToken(
  ...
  const headerToken =
    typeof req.headers["x-omniclaw-token"] === "string"
      ? req.headers["x-omniclaw-token"].trim()
      : "";
  if (headerToken) {
