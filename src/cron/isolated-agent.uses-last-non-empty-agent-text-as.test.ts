...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "omniclaw-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".omniclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OmniClawConfig> = {},
): OmniClawConfig {
  const base: OmniClawConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "omniclaw"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".omniclaw", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
