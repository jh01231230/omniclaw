import type { OmniClawConfig } from "./config.js";
...
function isTelegramConfigured(cfg: OmniClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: OmniClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: OmniClawConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: OmniClawConfig): boolean {
...
function isIMessageConfigured(cfg: OmniClawConfig): boolean {
...
function isWhatsAppConfigured(cfg: OmniClawConfig): boolean {
...
function isGenericChannelConfigured(cfg: OmniClawConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: OmniClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OmniClawConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: OmniClawConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: OmniClawConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: OmniClawConfig, pluginId: string): OmniClawConfig {
...
function enablePluginEntry(cfg: OmniClawConfig, pluginId: string): OmniClawConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
