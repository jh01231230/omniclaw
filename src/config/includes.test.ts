...
const ETC_OMNICLAW_DIR = path.join(ROOT_DIR, "etc", "omniclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "omniclaw.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcOmniClawPath(...parts: string[]) {
  return path.join(ETC_OMNICLAW_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcOmniClawPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "omniclaw.json"))).toEqual({
      shared: true,
    });
  });
});
