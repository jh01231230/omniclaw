...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-omniclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-omniclaw#general");
  });
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "omniclaw-sessions-"));
  ...
  it("derives session transcripts dir from OMNICLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OMNICLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.OMNICLAW_STATE_DIR = "/custom/state";
    try {
