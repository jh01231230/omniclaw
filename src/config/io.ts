...
import type { OmniClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareOmniClawVersions } from "./version.js";
...
function coerceConfig(value: unknown): OmniClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OmniClawConfig;
}
...
function stampConfigVersion(cfg: OmniClawConfig): OmniClawConfig {
...
function warnIfConfigFromFuture(cfg: OmniClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOmniClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OmniClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: OmniClawConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OmniClawConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as OmniClawConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OmniClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OmniClawConfig) {
    clearConfigCache();
...
// module scope. `OMNICLAW_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OMNICLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OMNICLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): OmniClawConfig {
...
export async function writeConfigFile(cfg: OmniClawConfig): Promise<void> {
  clearConfigCache();
