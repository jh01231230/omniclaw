...
import type { OmniClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OmniClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: OmniClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OmniClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OmniClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OmniClawConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as OmniClawConfig)),
    ),
  };
}
