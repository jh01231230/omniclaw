...
export const DEFAULT_LOG_DIR = "/tmp/omniclaw";
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "omniclaw.log"); // legacy single-file path

const LOG_PREFIX = "omniclaw";
...
function resolveSettings(): ResolvedSettings {
  let cfg: OmniClawConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => OmniClawConfig;
      };
