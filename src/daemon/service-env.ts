...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.OMNICLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.OMNICLAW_STATE_DIR;
  const configPath = env.OMNICLAW_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    OMNICLAW_PROFILE: profile,
    OMNICLAW_STATE_DIR: stateDir,
    OMNICLAW_CONFIG_PATH: configPath,
    OMNICLAW_GATEWAY_PORT: String(port),
    OMNICLAW_GATEWAY_TOKEN: token,
    OMNICLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    OMNICLAW_SYSTEMD_UNIT: systemdUnit,
    OMNICLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    OMNICLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    OMNICLAW_STATE_DIR: stateDir,
    OMNICLAW_CONFIG_PATH: configPath,
    OMNICLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    OMNICLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    OMNICLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    OMNICLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    OMNICLAW_LOG_PREFIX: "node",
    OMNICLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    OMNICLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    OMNICLAW_SERVICE_VERSION: VERSION,
  };
}
