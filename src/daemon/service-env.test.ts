...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OMNICLAW_GATEWAY_PORT).toBe("18789");
    expect(env.OMNICLAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.OMNICLAW_SERVICE_MARKER).toBe("omniclaw");
    expect(env.OMNICLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OMNICLAW_SERVICE_VERSION).toBe("string");
    expect(env.OMNICLAW_SYSTEMD_UNIT).toBe("omniclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OMNICLAW_LAUNCHD_LABEL).toBe("ai.omniclaw.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OMNICLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OMNICLAW_SYSTEMD_UNIT).toBe("omniclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OMNICLAW_LAUNCHD_LABEL).toBe("ai.omniclaw.work");
    }
  });
});
