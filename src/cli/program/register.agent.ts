...
${formatHelpExamples([
  ['omniclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['omniclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'omniclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'omniclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['omniclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'omniclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.omniclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.omniclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['omniclaw agents set-identity --agent main --name "OmniClaw" --emoji "🦞"', "Set name + emoji."],
  ["omniclaw agents set-identity --agent main --avatar avatars/omniclaw.png", "Set avatar path."],
  [
    "omniclaw agents set-identity --workspace ~/.omniclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "omniclaw agents set-identity --identity-file ~/.omniclaw/workspace/IDENTITY.md --agent main",
