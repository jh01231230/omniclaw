...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    OMNICLAW_STATE_DIR: process.env.OMNICLAW_STATE_DIR,
    OMNICLAW_CONFIG_PATH: process.env.OMNICLAW_CONFIG_PATH,
    OMNICLAW_GATEWAY_PORT: process.env.OMNICLAW_GATEWAY_PORT,
    OMNICLAW_PROFILE: process.env.OMNICLAW_PROFILE,
  };

  beforeEach(() => {
    process.env.OMNICLAW_STATE_DIR = "/tmp/omniclaw-cli-state";
    process.env.OMNICLAW_CONFIG_PATH = "/tmp/omniclaw-cli-state/omniclaw.json";
    delete process.env.OMNICLAW_GATEWAY_PORT;
    delete process.env.OMNICLAW_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.OMNICLAW_STATE_DIR !== undefined) {
      process.env.OMNICLAW_STATE_DIR = originalEnv.OMNICLAW_STATE_DIR;
    } else {
      delete process.env.OMNICLAW_STATE_DIR;
    }

    if (originalEnv.OMNICLAW_CONFIG_PATH !== undefined) {
      process.env.OMNICLAW_CONFIG_PATH = originalEnv.OMNICLAW_CONFIG_PATH;
    } else {
      delete process.env.OMNICLAW_CONFIG_PATH;
    }

    if (originalEnv.OMNICLAW_GATEWAY_PORT !== undefined) {
      process.env.OMNICLAW_GATEWAY_PORT = originalEnv.OMNICLAW_GATEWAY_PORT;
    } else {
      delete process.env.OMNICLAW_GATEWAY_PORT;
    }

    if (originalEnv.OMNICLAW_PROFILE !== undefined) {
      process.env.OMNICLAW_PROFILE = originalEnv.OMNICLAW_PROFILE;
    } else {
      delete process.env.OMNICLAW_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        OMNICLAW_PROFILE: "dev",
        OMNICLAW_STATE_DIR: "/tmp/omniclaw-daemon-state",
        OMNICLAW_CONFIG_PATH: "/tmp/omniclaw-daemon-state/omniclaw.json",
        OMNICLAW_GATEWAY_PORT: "19001",
      },
