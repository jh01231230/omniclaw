...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.omniclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.OMNICLAW_PROFILE = profile;

  const stateDir = env.OMNICLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.OMNICLAW_STATE_DIR?.trim()) {
    env.OMNICLAW_STATE_DIR = stateDir;
  }

  if (!env.OMNICLAW_CONFIG_PATH?.trim()) {
    env.OMNICLAW_CONFIG_PATH = path.join(stateDir, "omniclaw.json");
  }

  if (profile === "dev" && !env.OMNICLAW_GATEWAY_PORT?.trim()) {
    env.OMNICLAW_GATEWAY_PORT = "19001";
  }
}
