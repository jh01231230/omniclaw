...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "omniclaw-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OMNICLAW_NO_RESPAWN: "1",
      OMNICLAW_STATE_DIR: stateDir,
      OMNICLAW_SKIP_CHANNELS: "1",
      OMNICLAW_SKIP_GMAIL_WATCHER: "1",
      OMNICLAW_SKIP_CRON: "1",
      OMNICLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
      OMNICLAW_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "omniclaw-entry-bootstrap.mjs");
