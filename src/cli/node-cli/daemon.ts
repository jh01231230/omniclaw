...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("omniclaw node install"), formatCliCommand("omniclaw node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("omniclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    OMNICLAW_LOG_PREFIX: baseEnv.OMNICLAW_LOG_PREFIX ?? "node",
