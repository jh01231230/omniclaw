...
import { resolveOmniClawPackageRoot } from "../infra/omniclaw-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "omniclaw doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "omniclaw";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const OMNICLAW_REPO_URL = "https://github.com/omniclaw/omniclaw.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".omniclaw");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("omniclaw@")) {
    return trimmed.slice("omniclaw@".length);
  }
...
function resolveGitInstallDir(): string {
  const override = process.env.OMNICLAW_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OMNICLAW_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OMNICLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set OMNICLAW_GIT_DIR to an empty folder or an omniclaw checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OMNICLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveOmniClawPackageRoot({
  ...
  defaultRuntime.log(theme.heading("OmniClaw update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating OmniClaw..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this OmniClaw install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("omniclaw doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("omniclaw gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g omniclaw@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g omniclaw@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.OMNICLAW_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.OMNICLAW_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("omniclaw gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("omniclaw doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("omniclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("omniclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `omniclaw update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `OMNICLAW_GIT_DIR points at a non-git directory: ${gitDir}. Set OMNICLAW_GIT_DIR to an empty folder or an omniclaw checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via OMNICLAW_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update OmniClaw to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["omniclaw update", "Update a source checkout (git)"],
        ["omniclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["omniclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["omniclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["omniclaw update --no-restart", "Update without restarting the service"],
        ["omniclaw update --json", "Output result as JSON"],
        ["omniclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["omniclaw update wizard", "Interactive update wizard"],
        ["omniclaw --update", "Shorthand for omniclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run omniclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.omniclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["omniclaw update status", "Show channel + version status."],
          ["omniclaw update status --json", "JSON output."],
          ["omniclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.omniclaw.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
