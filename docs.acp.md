...
This document describes how the OmniClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`omniclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
OmniClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a OmniClaw Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `omniclaw acp` over stdio.

Example config:

```bash
omniclaw config set gateway.remote.url wss://gateway-host:18789
omniclaw config set gateway.remote.token <token>
```

Example run:

```bash
omniclaw acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
omniclaw acp --session agent:main:main
omniclaw acp --session agent:design:main
omniclaw acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “OmniClaw ACP” to start a thread.

## Execution Model

- ACP client spawns `omniclaw acp` and speaks ACP messages over stdio.
...
```bash
omniclaw acp --session agent:main:main
omniclaw acp --session-label "support inbox"
omniclaw acp --reset-session
```
...
`omniclaw acp` resolves the Gateway URL and auth from CLI flags or config:
