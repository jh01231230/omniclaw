...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OMNICLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.OMNICLAW_STATE_DIR = path.join(base, ".omniclaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "omniclaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".omniclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
