...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `omniclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".omniclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OMNICLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        OMNICLAW_GATEWAY_TOKEN: inst.gatewayToken,
        OMNICLAW_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OMNICLAW_GATEWAY_PORT: String(gwA.port),
          OMNICLAW_GATEWAY_TOKEN: gwA.gatewayToken,
          OMNICLAW_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OMNICLAW_GATEWAY_PORT: String(gwB.port),
          OMNICLAW_GATEWAY_TOKEN: gwB.gatewayToken,
          OMNICLAW_GATEWAY_PASSWORD: "",
        }),
