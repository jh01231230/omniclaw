...
import type { OmniClawConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("omniclaw-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("omniclaw-bin-sherpa-");
      const modelDir = await makeTempDir("omniclaw-sherpa-model-");
      ...
      const cfg: OmniClawConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("omniclaw-bin-whispercpp-");
      const modelDir = await makeTempDir("omniclaw-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("omniclaw-bin-gemini-");
      ...
      const cfg: OmniClawConfig = { tools: { media: { image: {} } } };
