...
declare global {
  interface Window {
    __OMNICLAW_ASSISTANT_NAME__?: string;
    __OMNICLAW_ASSISTANT_AVATAR__?: string;
  }
}
...
export function resolveInjectedAssistantIdentity(): AssistantIdentity {
  ...
  return normalizeAssistantIdentity({
    name: window.__OMNICLAW_ASSISTANT_NAME__,
    avatar: window.__OMNICLAW_ASSISTANT_AVATAR__,
  });
}
