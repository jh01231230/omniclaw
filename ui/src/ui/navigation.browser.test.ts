...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/omniclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/omniclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/omniclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OMNICLAW_CONTROL_UI_BASE_PATH__ = "/omniclaw";
    const app = mountApp("/omniclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/omniclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/omniclaw/sessions");
  });
