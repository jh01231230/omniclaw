...
import type { OmniClawPluginApi, OmniClawPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "omniclaw-lobster-plugin-") {
...
function fakeApi(overrides: Partial<OmniClawPluginApi> = {}): OmniClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<OmniClawPluginToolContext> = {}): OmniClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: OmniClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
