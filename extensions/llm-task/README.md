...
`omniclaw.invoke --each`) without adding new OmniClaw code per workflow.
...
This extension depends on OmniClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OmniClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.omniclaw/extensions` as a standalone plugin directory.
