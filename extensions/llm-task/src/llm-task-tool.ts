...
// NOTE: This extension is intended to be bundled with OmniClaw.
// When running from source (tests/dev), OmniClaw internals live under src/.
...
export function createLlmTaskTool(api: OmniClawPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via omniclaw.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "omniclaw-llm-task-"));
