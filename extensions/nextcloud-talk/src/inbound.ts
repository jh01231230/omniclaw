...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as OmniClawConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as OmniClawConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as OmniClawConfig);
