...
OmniClaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
omniclaw channels login --channel zalouser
...
```bash
omniclaw message send --channel zalouser --target <threadId> --message "Hello from OmniClaw!"
```
...
```bash
omniclaw channels login --channel zalouser              # Login via QR
omniclaw channels login --channel zalouser --account work
omniclaw channels status --probe
omniclaw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
omniclaw directory self --channel zalouser
omniclaw directory peers list --channel zalouser --query "name"
omniclaw directory groups list --channel zalouser --query "work"
omniclaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
omniclaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
omniclaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `omniclaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
omniclaw channels login --channel zalouser --account work
omniclaw message send --channel zalouser --account work --target <id> --message "Hello"
