...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OMNICLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "omniclaw-nostr-"));
  process.env.OMNICLAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.OMNICLAW_STATE_DIR?.trim() || env.OMNICLAW_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".omniclaw");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OMNICLAW_STATE_DIR = previous;
    }
