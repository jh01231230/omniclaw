/**
 * Twitch channel plugin for OmniClaw.
 *
...
import { buildChannelConfigSchema } from "omniclaw/plugin-sdk";
import type {
 ...
 * for OmniClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OmniClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OmniClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OmniClawConfig): boolean => {
