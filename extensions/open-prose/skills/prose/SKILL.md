...
metadata: { "omniclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OmniClaw `sessions_spawn`
- **File I/O** == OmniClaw `read`/`write`
- **Remote fetch** == OmniClaw `web_fetch` (or `exec` with curl when POST is required)
