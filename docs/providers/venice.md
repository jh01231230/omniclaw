---
summary: "Use Venice AI privacy-focused models in OmniClaw"
read_when:
  - You want privacy-focused inference in OmniClaw
...
```bash
omniclaw onboard --auth-choice venice-api-key
```
...
```bash
omniclaw chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, OmniClaw shows all available Venice models. Pick based on your needs:
...
```bash
omniclaw models set venice/claude-opus-45
omniclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
omniclaw models list | grep venice
```

## Configure via `omniclaw configure`

1. Run `omniclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OmniClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
omniclaw chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
omniclaw chat --model venice/claude-opus-45

# Use uncensored model
omniclaw chat --model venice/venice-uncensored

# Use vision model with image
omniclaw chat --model venice/qwen3-vl-235b-a22b

# Use coding model
omniclaw chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `omniclaw models list` to see currently available models. Some models may be temporarily offline.
