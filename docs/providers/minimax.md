---
summary: "Use MiniMax M2.1 in OmniClaw"
read_when:
  - You want MiniMax models in OmniClaw
...
```bash
omniclaw plugins enable minimax-portal-auth  # skip if already loaded.
omniclaw gateway restart  # restart if gateway is already running
omniclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/omniclaw/omniclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `omniclaw.json`:
...
- Use `omniclaw models list` and `omniclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `omniclaw configure` and selecting **MiniMax M2.1**, or
