...
- Jobs persist under `~/.omniclaw/cron/` so restarts don’t lose schedules.
...
omniclaw cron list
omniclaw cron run <job-id> --force
omniclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.omniclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `omniclaw cron add/edit` or the cron
...
- Job store: `~/.omniclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.omniclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
omniclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
omniclaw cron edit <jobId> --agent ops
omniclaw cron edit <jobId> --clear-agent
```
...
```bash
omniclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`omniclaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OMNICLAW_SKIP_CRON`.
