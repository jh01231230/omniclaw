...
- 你可以在 Gateway网关机器上（或通过 SSH）运行 CLI（`omniclaw`）。
...
- 在 Gateway网关主机的 `~/.omniclaw/omniclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway网关主机上设置 DNS-SD 区域（示例 `omniclaw.internal.`）并发布 `_openclaw-gw._tcp` 记录。
...
1. 在 Gateway网关主机上创建 `~/.omniclaw/workspace/canvas/index.html`。
