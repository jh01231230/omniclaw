...
- `~/Library/Application Support/OmniClaw/canvas/<session>/...`

Canvas 面板通过**自定义 URL scheme** 提供这些文件：

- `omniclaw-canvas://<session>/<path>`

示例：

- `omniclaw-canvas://main/` → `<canvasRoot>/main/index.html`
- `omniclaw-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `omniclaw-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
omniclaw nodes canvas present --node <id>
omniclaw nodes canvas navigate --node <id> --url "/"
omniclaw nodes canvas eval --node <id> --js "document.title"
omniclaw nodes canvas snapshot --node <id>
```
...
omniclaw nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速冒烟测试：

```bash
omniclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "omniclaw://agent?message=Review%20this%20design";
```
