...
- 网络路径：
  - 通过 Bonjour 的同一局域网，**或**
  - 通过单播 DNS-SD 的 Tailnet（示例域名：`omniclaw.internal.`），**或**
  - 手动输入主机/端口（备用方案）。
...
如果 mDNS 被阻止，请使用单播 DNS-SD 区域（选择一个域名；示例：`omniclaw.internal.`）和 Tailscale 分割 DNS。
...
- 配对提示始终未出现：运行 `omniclaw nodes pending` 并手动批准。
