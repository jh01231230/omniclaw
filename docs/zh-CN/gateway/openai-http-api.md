...
OmniClaw 的 Gateway网关可以提供一个小型的兼容 OpenAI 的 Chat Completions 端点。
...
在底层，请求会作为普通的 Gateway网关智能体运行来执行（与 `omniclaw agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway网关保持一致。
...
- 当 `gateway.auth.mode="token"` 时，使用 `gateway.auth.token`（或 `OMNICLAW_GATEWAY_TOKEN`）。
- 当 `gateway.auth.mode="password"` 时，使用 `gateway.auth.password`（或 `OMNICLAW_GATEWAY_PASSWORD`）。

## 选择智能体

无需自定义请求头：在 OpenAI 的 `model` 字段中编码智能体 ID：

- `model: "omniclaw:<agentId>"`（示例：`"omniclaw:main"`、`"omniclaw:beta"`）
- `model: "agent:<agentId>"`（别名）

或通过请求头指定特定的 OmniClaw 智能体：

- `x-omniclaw-agent-id: <agentId>`（默认值：`main`）

高级用法：

- `x-omniclaw-session-key: <sessionKey>` 用于完全控制会话路由。
