...
OmniClaw 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现活跃的 Gateway网关（WebSocket 端点）。这是尽力而为的机制，**不能**替代 SSH 或基于 Tailnet 的连接。
...
2. 在专用区域下为 `_openclaw-gw._tcp` 发布 DNS‑SD 记录（示例：`omniclaw.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该 DNS 服务器为客户端（包括 iOS）解析。

OmniClaw 支持任意发现域名；`omniclaw.internal.` 仅为示例。iOS/Android 节点会同时浏览 `local.` 和你配置的广域域名。
...
- 从 `~/.omniclaw/dns/<domain>.db` 提供你选择的域名服务（示例：`omniclaw.internal.`）
...
- 在 `~/.omniclaw/omniclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
- `transport=gateway`
- `cliPath=<路径>`（可选；可运行的 `omniclaw` 入口点的绝对路径）
...
- `OMNICLAW_DISABLE_BONJOUR=1` 禁用广播（旧版：`OMNICLAW_DISABLE_BONJOUR`）。
- `~/.omniclaw/omniclaw.json` 中的 `gateway.bind` 控制 Gateway网关绑定模式。
- `OMNICLAW_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（旧版：`OMNICLAW_SSH_PORT`）。
- `OMNICLAW_TAILNET_DNS` 在 TXT 中发布 MagicDNS 提示（旧版：`OMNICLAW_TAILNET_DNS`）。
- `OMNICLAW_CLI_PATH` 覆盖广播的 CLI 路径（旧版：`OMNICLAW_CLI_PATH`）。
