...
summary: 在同一主机上运行多个 OmniClaw Gateway网关（隔离、端口和配置文件）
title: 多个 Gateway网关
x-i18n:
...
- `OMNICLAW_CONFIG_PATH` — 每个实例独立的配置文件
- `OMNICLAW_STATE_DIR` — 每个实例独立的会话、凭据、缓存
...
配置文件会自动限定 `OMNICLAW_STATE_DIR` + `OMNICLAW_CONFIG_PATH` 的作用域，并为服务名称添加后缀。
