...
- **会话存储**（聊天历史 + 路由状态），位于 `~/.omniclaw/agents/<agentId>/sessions`。
...
Skills 通过每个工作区的 `skills/` 文件夹按智能体隔离，共享 Skills 可从 `~/.omniclaw/skills` 获取。参阅[Skills：按智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 配置文件：`~/.omniclaw/omniclaw.json`（或 `OMNICLAW_CONFIG_PATH`）
- 状态目录：`~/.omniclaw`（或 `OMNICLAW_STATE_DIR`）
- 工作区：`~/.omniclaw/workspace`（或 `~/.omniclaw/workspace-<agentId>`）
- 智能体目录：`~/.omniclaw/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.omniclaw/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你不做任何配置，OmniClaw 会运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键格式为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.omniclaw/workspace`（设置了 `OMNICLAW_PROFILE` 时为 `~/.omniclaw/workspace-<profile>`）。
- 状态默认为 `~/.omniclaw/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认值：~/.omniclaw/credentials/whatsapp/personal
          // authDir: "~/.omniclaw/credentials/whatsapp/personal",
        },
        biz: {
          // 可选覆盖。默认值：~/.omniclaw/credentials/whatsapp/biz
