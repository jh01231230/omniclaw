...
使用 Nix 运行 OmniClaw 的推荐方式是通过 **[nix-omniclaw](https://github.com/omniclaw/nix-omniclaw)** — 一个开箱即用的 Home Manager 模块。
...
> **📦 完整指南：[github.com/omniclaw/nix-omniclaw](https://github.com/omniclaw/nix-omniclaw)**
>
> nix-omniclaw 仓库是 Nix 安装的权威来源。本页仅为简要概览。
...
当设置了 `OMNICLAW_NIX_MODE=1` 时（nix-omniclaw 会自动设置）：

OmniClaw 支持 **Nix 模式**，使配置具有确定性并禁用自动安装流程。
通过导出以下环境变量启用：
...
OmniClaw 从 `OMNICLAW_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `OMNICLAW_STATE_DIR` 中。

- `OMNICLAW_STATE_DIR`（默认：`~/.omniclaw`）
- `OMNICLAW_CONFIG_PATH`（默认：`$OMNICLAW_STATE_DIR/omniclaw.json`）
...
[`scripts/package-mac-app.sh`](https://github.com/omniclaw/omniclaw/blob/main/scripts/package-mac-app.sh) 将此模板复制到应用包中并修补动态字段
（bundle ID、版本/构建号、Git SHA、Sparkle 密钥）。这使得 plist 对 SwiftPM
打包和 Nix 构建保持确定性（它们不依赖完整的 Xcode 工具链）。

## 相关内容

- [nix-omniclaw](https://github.com/omniclaw/nix-omniclaw) — 完整设置指南
