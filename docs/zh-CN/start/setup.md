...
summary: 设置指南：在保持更新的同时维护你的个性化 OmniClaw 配置
title: 设置
x-i18n:
...
- **个性化配置存放在仓库之外：** `~/.omniclaw/workspace`（工作区）+ `~/.omniclaw/omniclaw.json`（配置）。
...
- **配置：** `~/.omniclaw/omniclaw.json`（JSON/JSON5 风格）
- **工作区：** `~/.omniclaw/workspace`（Skills、提示词、记忆；建议设为私有 git 仓库）
...
如果你还没有全局安装，请通过 `pnpm omniclaw setup` 运行。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **OmniClaw.app**（菜单栏）。
...
- 运行 `omniclaw setup`，然后 `omniclaw channels login`，接着手动启动 Gateway网关（`omniclaw gateway`）。
...
### 2) 将 macOS 应用指向你运行的 Gateway网关在 **OmniClaw.app** 中：
...
- **模型认证配置文件**：`~/.omniclaw/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.omniclaw/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的配置）

- 将 `~/.omniclaw/workspace` 和 `~/.omniclaw/` 视为"你的东西"；不要将个人提示词/配置放入 `omniclaw` 仓库中。
