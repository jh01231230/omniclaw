...
当你需要切换一些不常用的设置而又不想编辑 `omniclaw.json` 时，这非常方便。
...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.omniclaw-dev`，并将 Gateway网关默认端口设为 `19001`（派生端口随之偏移）。
...
如果你还没有全局安装，可通过 `pnpm omniclaw ...` 运行 CLI。
...
1. **配置文件隔离**（全局 `--dev`）
   ...
   - `OMNICLAW_GATEWAY_PORT=19001`（浏览器/画布端口随之偏移）
...
2. **开发引导**（`gateway --dev`）
   ...
   - 默认身份：**C3‑PO**（礼仪机器人）。
   - 在开发模式下跳过渠道提供商（`OMNICLAW_SKIP_CHANNELS=1`）。
...
OmniClaw 可以在任何过滤/格式化之前记录**原始助手流**。
