...
你可以在 `~/.omniclaw/omniclaw.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/omniclaw.log"
  }
}
```
...
```bash
omniclaw channels logs --channel whatsapp
```
...
所有日志配置位于 `~/.omniclaw/omniclaw.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/omniclaw/omniclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- OmniClaw 目前通过 **OTLP/HTTP (protobuf)** 导出。
...
```
OMNICLAW_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `omniclaw plugins enable diagnostics-otel` 来启用插件。
...
- `omniclaw.tokens`（计数器，属性：`omniclaw.token`、`omniclaw.channel`、`omniclaw.provider`、`omniclaw.model`）
- `omniclaw.cost.usd`（计数器，属性：`omniclaw.channel`、`omniclaw.provider`、`omniclaw.model`）
- `omniclaw.run.duration_ms`（直方图，属性：`omniclaw.channel`、`omniclaw.provider`、`omniclaw.model`）
- `omniclaw.context.tokens`（直方图，属性：`omniclaw.context`、`omniclaw.channel`、`omniclaw.provider`、`omniclaw.model`）

消息流：

- `omniclaw.webhook.received`（计数器，属性：`omniclaw.channel`、`omniclaw.webhook`）
- `omniclaw.webhook.error`（计数器，属性：`omniclaw.channel`、`omniclaw.webhook`）
- `omniclaw.webhook.duration_ms`（直方图，属性：`omniclaw.channel`、`omniclaw.webhook`）
- `omniclaw.message.queued`（计数器，属性：`omniclaw.channel`、`omniclaw.source`）
- `omniclaw.message.processed`（计数器，属性：`omniclaw.channel`、`omniclaw.outcome`）
- `omniclaw.message.duration_ms`（直方图，属性：`omniclaw.channel`、`omniclaw.outcome`）

队列 + 会话：

- `omniclaw.queue.lane.enqueue`（计数器，属性：`omniclaw.lane`）
- `omniclaw.queue.lane.dequeue`（计数器，属性：`omniclaw.lane`）
- `omniclaw.queue.depth`（直方图，属性：`omniclaw.lane` 或 `omniclaw.channel=heartbeat`）
- `omniclaw.queue.wait_ms`（直方图，属性：`omniclaw.lane`）
- `omniclaw.session.state`（计数器，属性：`omniclaw.state`、`omniclaw.reason`）
- `omniclaw.session.stuck`（计数器，属性：`omniclaw.state`）
- `omniclaw.session.stuck_age_ms`（直方图，属性：`omniclaw.state`）
- `omniclaw.run.attempt`（计数器，属性：`omniclaw.attempt`）

### 导出的 span（名称 + 关键属性）

- `omniclaw.model.usage`
  - `omniclaw.channel`、`omniclaw.provider`、`omniclaw.model`
  - `omniclaw.sessionKey`、`omniclaw.sessionId`
  - `omniclaw.tokens.*`（input/output/cache_read/cache_write/total）
- `omniclaw.webhook.processed`
  - `omniclaw.channel`、`omniclaw.webhook`、`omniclaw.chatId`
- `omniclaw.webhook.error`
  - `omniclaw.channel`、`omniclaw.webhook`、`omniclaw.chatId`、`omniclaw.error`
- `omniclaw.message.processed`
  - `omniclaw.channel`、`omniclaw.outcome`、`omniclaw.chatId`、`omniclaw.messageId`、`omniclaw.sessionKey`、`omniclaw.sessionId`、`omniclaw.reason`
- `omniclaw.session.stuck`
  - `omniclaw.state`、`omniclaw.ageMs`、`omniclaw.queueDepth`、`omniclaw.sessionKey`、`omniclaw.sessionId`
...
- **Gateway网关不可达？** 先运行 `omniclaw doctor`。
