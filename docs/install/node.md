...
read_when:
  - "You installed OmniClaw but `omniclaw` is “command not found”"
...
OmniClaw’s runtime baseline is **Node 22+**.

If you can run `npm install -g omniclaw@latest` but later see `omniclaw: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
