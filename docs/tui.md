...
```bash
omniclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `omniclaw tui --deliver`
...
- Check the Gateway logs: `omniclaw logs --follow`.
- Confirm the agent can run: `omniclaw status` and `omniclaw models status`.
...
- No agents in picker: check `omniclaw agents list` and your routing config.
