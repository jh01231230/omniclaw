...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/omniclaw`)
...
If the page fails to load, start the Gateway first: `omniclaw gateway`.
...
you revoke it with `omniclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.omniclaw/omniclaw.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OmniClaw
...
```bash
omniclaw gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
OmniClaw **blocks** Control UI connections without device identity.
...
```bash
OMNICLAW_CONTROL_UI_BASE_PATH=/omniclaw/ pnpm ui:build
```
