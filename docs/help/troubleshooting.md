...
You can also set `OMNICLAW_VERBOSE=1` instead of the flag.
...
### `docs.omniclaw.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.omniclaw.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.omniclaw.ai` to the allowlist, then retry.
...
- Check the allowlist: `omniclaw config get agents.defaults.models`
...
If you can, include the relevant log tail from `omniclaw logs --follow`.
