...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `omniclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `omniclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=omniclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${OMNICLAW_STATE_DIR:-$HOME/.omniclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
