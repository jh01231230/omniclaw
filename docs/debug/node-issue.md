...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OmniClaw
...
Running OmniClaw via Node with `tsx` fails at startup with:

```
[omniclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `omniclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch omniclaw.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node omniclaw.mjs status` works on Node 25.
