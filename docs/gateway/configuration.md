---
summary: "All configuration options for ~/.omniclaw/omniclaw.json with examples"
read_when:
...
OmniClaw reads an optional **JSON5** config from `~/.omniclaw/omniclaw.json` (comments + trailing commas allowed).

If the file is missing, OmniClaw uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.omniclaw/workspace`). You usually only need a config to:
...
OmniClaw only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `omniclaw doctor`, `omniclaw logs`, `omniclaw health`, `omniclaw status`, `omniclaw service`, `omniclaw help`).
- Run `omniclaw doctor` to see the exact issues.
- Run `omniclaw doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `omniclaw config set`. Keep a backup of `~/.omniclaw/omniclaw.json`.
...
```bash
omniclaw gateway call config.get --params '{}' # capture payload.hash
omniclaw gateway call config.apply --params '{
...
```bash
omniclaw gateway call config.get --params '{}' # capture payload.hash
omniclaw gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.omniclaw/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@omniclaw", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.omniclaw/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.omniclaw/workspace" }],
}
```
...
{ "$include": "/etc/omniclaw/base.json5" } // absolute
...
```json5
// ~/.omniclaw/clients/mueller/agents.json5
[
...
```json5
// ~/.omniclaw/clients/mueller/broadcast.json5
{
...
OmniClaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, OmniClaw runs your login shell and imports only the missing expected keys (never overrides).
...
- `OMNICLAW_LOAD_SHELL_ENV=1`
- `OMNICLAW_SHELL_ENV_TIMEOUT_MS=15000`
...
OmniClaw stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.omniclaw/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.omniclaw/credentials/oauth.json` (or `$OMNICLAW_STATE_DIR/credentials/oauth.json`)
...
- `~/.omniclaw/agent/*` (migrated by `omniclaw doctor` into `~/.omniclaw/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `OMNICLAW_OAUTH_DIR`
- Agent dir (default agent root override): `OMNICLAW_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, OmniClaw imports `oauth.json` entries into `auth-profiles.json`.
...
If set, OmniClaw derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/omniclaw/omniclaw-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/omniclaw/omniclaw.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/omniclaw/omniclaw.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.omniclaw/credentials/whatsapp/biz
          // authDir: "~/.omniclaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `omniclaw doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@omniclaw", "omniclaw"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@omniclaw"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.omniclaw/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.omniclaw/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.omniclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.omniclaw/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.omniclaw/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.omniclaw/workspace-home" },
      { id: "work", workspace: "~/.omniclaw/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `omniclaw.json`).
...
OmniClaw starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["omniclaw-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-omniclaw",
...
OmniClaw starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `omniclaw plugins install @omniclaw/mattermost` (or `./extensions/mattermost` from a git checkout).
...
OmniClaw starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
OmniClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, OmniClaw
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/omniclaw" } },
}
```
...
When a file exceeds this limit, OmniClaw logs a warning and injects a truncated
...
message envelopes). If unset, OmniClaw uses the host timezone at runtime.
...
`[omniclaw]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[omniclaw]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[omniclaw]"`, OmniClaw will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, OmniClaw generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.omniclaw/settings/tts.json",
      elevenlabs: {
...
OmniClaw also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, OmniClaw currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `omniclaw configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:omniclaw`: all built-in OmniClaw tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.omniclaw/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.omniclaw/sandboxes",
        docker: {
          image: "omniclaw-sandbox:bookworm-slim",
          containerPrefix: "omniclaw-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "omniclaw-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "omniclaw-sandbox-browser:bookworm-slim",
          containerPrefix: "omniclaw-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
OmniClaw uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.omniclaw/agents/<agentId>/agent/models.json` or by defining the same schema inside your
OmniClaw config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, OmniClaw writes/merges a `models.json` into
`~/.omniclaw/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. OmniClaw uses
...
- Shortcut: `omniclaw onboard --auth-choice opencode-zen`.
...
Shortcut: `omniclaw onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `omniclaw onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `omniclaw onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `omniclaw onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `omniclaw onboard --auth-choice minimax-api`.
...
- Override the agent config root with `OMNICLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.omniclaw/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.omniclaw/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.omniclaw/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, OmniClaw stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.omniclaw/skills` (workspace skills
...
from `~/.omniclaw/extensions`, `<workspace>/.omniclaw/extensions`, plus any
...
### `browser` (omniclaw-managed browser)

OmniClaw can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for omniclaw and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (OmniClaw.app menubar, or `omniclaw gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      omniclaw: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/omniclaw" }
...
- Examples: `"/ui"`, `"/omniclaw"`, `"/apps/omniclaw"`.
...
- When a connection comes from one of these IPs, OmniClaw uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `omniclaw gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `OMNICLAW_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `OMNICLAW_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. OmniClaw
...
- OmniClaw.app watches `~/.omniclaw/omniclaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.omniclaw/omniclaw.json` (or `OMNICLAW_CONFIG_PATH`) and applies changes automatically.
...
- `~/.omniclaw/omniclaw.json` (or `OMNICLAW_CONFIG_PATH`)
...
- `OMNICLAW_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `omniclaw --dev …` → uses `~/.omniclaw-dev` + shifts ports from base `19001`
- `omniclaw --profile <name> …` → uses `~/.omniclaw-<name>` (port via config/env/flags)
...
```bash
OMNICLAW_CONFIG_PATH=~/.omniclaw/a.json \
OMNICLAW_STATE_DIR=~/.omniclaw-a \
...
Gmail helper config (used by `omniclaw webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "omniclaw@gmail.com",
...
- Set `OMNICLAW_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, OmniClaw defaults `serve.path` to `/` so
...
Default root: `~/.omniclaw/workspace/canvas`  
Default port: `18793` (chosen to avoid the omniclaw browser CDP port `18792`)  
...
{
  canvasHost: {
    root: "~/.omniclaw/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `OMNICLAW_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.omniclaw/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.omniclaw/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.omniclaw/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
- Hostname: defaults to `omniclaw` (advertises `omniclaw.local`). Override with `OMNICLAW_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_openclaw-gw._tcp` under `~/.omniclaw/dns/` using the configured discovery domain (example: `omniclaw.internal.`).
