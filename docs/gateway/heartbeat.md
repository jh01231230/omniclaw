...
- During heartbeat runs, OmniClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `omniclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OmniClaw skips the heartbeat run entirely (no model call).
...
```bash
omniclaw system event --text "Check for urgent follow-ups" --mode now
```
