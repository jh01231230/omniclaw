---
summary: "Quick troubleshooting guide for common OmniClaw failures"
read_when:
...
When OmniClaw misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `omniclaw status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `omniclaw status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `omniclaw status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `omniclaw gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `omniclaw channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `omniclaw gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `omniclaw logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `omniclaw status --all` (it redacts tokens). If you paste `omniclaw status`, consider setting `OMNICLAW_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  omniclaw models auth setup-token --provider anthropic
  ```
...
```bash
omniclaw models auth paste-token --provider anthropic
omniclaw models status
```
...
- Preferred: `omniclaw logs --follow`
- File logs (always): `/tmp/omniclaw/omniclaw-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$OMNICLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u omniclaw-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  omniclaw config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  omniclaw config set gateway.mode remote
  omniclaw config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `omniclaw setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.omniclaw/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `omniclaw doctor`
...
- then run `omniclaw sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `omniclaw gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `omniclaw gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `omniclaw gateway status` and `omniclaw doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `OMNICLAW_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `omniclaw gateway status` shows a config mismatch**
...
- Fix: rerun `omniclaw gateway install --force` from the same `--profile` / `OMNICLAW_STATE_DIR` you want the service to use.

**If `omniclaw gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `omniclaw doctor` to update it (or `omniclaw gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `OMNICLAW_GATEWAY_TOKEN`) and restart the service.

**If `omniclaw gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/omniclaw` on disk.
...
Symptoms: `pwd` or file tools show `~/.omniclaw/sandboxes/...` even though you
...
OmniClaw intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `omniclaw models list` or
  `omniclaw models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${OMNICLAW_CONFIG_PATH:-$HOME/.omniclaw/omniclaw.json}"
```
...
tail -f "$(ls -t /tmp/omniclaw/omniclaw-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
omniclaw logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.omniclaw/agents/<agentId>/sessions/
```
...
omniclaw logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
omniclaw gateway --verbose
```

If you’re logged out / unlinked:

```bash
omniclaw channels logout
trash "${OMNICLAW_STATE_DIR:-$HOME/.omniclaw}/credentials" # if logout can't cleanly remove everything
omniclaw channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/omniclaw/omniclaw-*.log | head -1)" | tail -20
```

### High Memory Usage

OmniClaw keeps conversation history in memory.
...
OmniClaw now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `omniclaw doctor` reports every invalid entry.
- `omniclaw doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `omniclaw logs`, `omniclaw health`, `omniclaw status`, `omniclaw gateway status`, and `omniclaw gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/omniclaw/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `omniclaw models status` (CLI).
...
```bash
curl -fsSL https://omniclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
```
...
5. Run `omniclaw channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. OmniClaw scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/omniclaw/omniclaw/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.omniclaw.* still works)
```
...
Ensure the global `omniclaw` CLI is installed and matches the app version:

```bash
omniclaw --version
npm install -g omniclaw@<version>
```
...
#   ${OMNICLAW_CONFIG_PATH:-$HOME/.omniclaw/omniclaw.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
omniclaw gateway --verbose
omniclaw channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/omniclaw/omniclaw-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$OMNICLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.omniclaw/logs/...`; profiles use `~/.omniclaw-<profile>/logs/...`)<br />Linux: `journalctl --user -u omniclaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "OmniClaw Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$OMNICLAW_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$OMNICLAW_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$OMNICLAW_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
omniclaw gateway status --deep

# Is the gateway reachable?
omniclaw health --json
# If it fails, rerun with connection details:
omniclaw health --verbose
...
tail -20 /tmp/omniclaw/omniclaw-*.log
```
...
# omniclaw gateway uninstall

trash "${OMNICLAW_STATE_DIR:-$HOME/.omniclaw}"
omniclaw channels login         # re-pair WhatsApp
omniclaw gateway restart           # or: omniclaw gateway
```
...
1. Check logs first: `/tmp/omniclaw/` (default: `omniclaw-YYYY-MM-DD.log`, or your configured `logging.file`)
