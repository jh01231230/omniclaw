...
- Replaces the legacy `gateway` command. CLI entry point: `omniclaw gateway`.
...
omniclaw gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
omniclaw gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.omniclaw/omniclaw.json` (or `OMNICLAW_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__openclaw__/canvas/` from `~/.omniclaw/workspace/canvas`. Disable with `canvasHost.enabled=false` or `OMNICLAW_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `OMNICLAW_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `OMNICLAW_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.omniclaw.*` may still exist)
- Linux: `omniclaw-gateway-<profile>.service`
- Windows: `OmniClaw Gateway (<profile>)`

Install metadata is embedded in the service config:

- `OMNICLAW_SERVICE_MARKER=omniclaw`
- `OMNICLAW_SERVICE_KIND=gateway`
- `OMNICLAW_SERVICE_VERSION=<version>`
...
```bash
omniclaw --dev setup
omniclaw --dev gateway --allow-unconfigured
...
- `OMNICLAW_STATE_DIR=~/.omniclaw-dev`
- `OMNICLAW_CONFIG_PATH=~/.omniclaw-dev/omniclaw.json`
- `OMNICLAW_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.omniclaw/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `OMNICLAW_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `OMNICLAW_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `OMNICLAW_CONFIG_PATH`
- unique `OMNICLAW_STATE_DIR`
...
```bash
OMNICLAW_CONFIG_PATH=~/.omniclaw/a.json OMNICLAW_STATE_DIR=~/.omniclaw-a omniclaw gateway --port 19001
OMNICLAW_CONFIG_PATH=~/.omniclaw/b.json OMNICLAW_STATE_DIR=~/.omniclaw-b omniclaw gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `omniclaw health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `omniclaw`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `omniclaw gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.omniclaw.*` is cleaned up).
  - `omniclaw doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are OmniClaw profile services.
  ...
  - Cleanup: `omniclaw gateway uninstall` (current service) and `omniclaw doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `omniclaw gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- OmniClaw.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.omniclaw.*` labels still unload cleanly).
- To stop it cleanly, use `omniclaw gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `omniclaw gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `omniclaw gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

OmniClaw installs a **systemd user service** by default on Linux/WSL2. We
...
`omniclaw gateway install` writes the user unit. `omniclaw doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/omniclaw-gateway[-<profile>].service`:

```
[Unit]
Description=OmniClaw Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/omniclaw gateway --port 18789
Restart=always
RestartSec=5
Environment=OMNICLAW_GATEWAY_TOKEN=
...
```
systemctl --user enable --now omniclaw-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/omniclaw-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now omniclaw-gateway[-<profile>].service
```
...
- `omniclaw gateway health|status` — request health/status over the Gateway WS.
- `omniclaw message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `omniclaw agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `omniclaw gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `omniclaw gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `omniclaw gateway` and the legacy TCP control port.
