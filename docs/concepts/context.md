...
read_when:
  - You want to understand what “context” means in OmniClaw
...
“Context” is **everything OmniClaw sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (OmniClaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How OmniClaw builds the system prompt

The system prompt is **OmniClaw-owned** and rebuilt each run. It includes:
...
By default, OmniClaw injects a fixed set of workspace files (if present):
