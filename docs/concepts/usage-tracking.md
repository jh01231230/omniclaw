...
- `/usage cost` in chats: local cost summary aggregated from OmniClaw session logs.
- CLI: `omniclaw status --usage` prints a full per-provider breakdown.
- CLI: `omniclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
