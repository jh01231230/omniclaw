...
```bash
omniclaw plugins install @omniclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
omniclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "OmniClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `omniclaw pairing list nextcloud-talk`
  - `omniclaw pairing approve nextcloud-talk <CODE>`
