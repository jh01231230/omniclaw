...
```bash
omniclaw plugins install @omniclaw/msteams
```

Local checkout (when running from a git repo):

```bash
omniclaw plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
OmniClaw will offer the local install path automatically.
...
3. Configure OmniClaw with those credentials.
...
- Talk to OmniClaw via Teams DMs, group chats, or channels.
...
- On startup, OmniClaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.omniclaw/omniclaw.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring OmniClaw, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `omniclaw-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `omniclaw plugins install @omniclaw/msteams`
   - From a local checkout: `omniclaw plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "OmniClaw in Teams", "full": "OmniClaw in Teams" },
...
OmniClaw handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, OmniClaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/OmniClawShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

OmniClaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `omniclaw message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.omniclaw/msteams-polls.json`.
...
```bash
omniclaw message send --channel msteams \
...
omniclaw message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
omniclaw message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
omniclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
omniclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
