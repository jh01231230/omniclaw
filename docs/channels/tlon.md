...
Tlon is a decentralized messenger built on Urbit. OmniClaw connects to your Urbit ship and can
...
```bash
omniclaw plugins install @omniclaw/tlon
```

Local checkout (when running from a git repo):

```bash
omniclaw plugins install ./extensions/tlon
```
...
Use these with `omniclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OmniClaw replies in-thread.
