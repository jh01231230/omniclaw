...
read_when:
  - Setting up Twitch chat integration for OmniClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OmniClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
omniclaw plugins install @omniclaw/twitch
```

Local checkout (when running from a git repo):

```bash
omniclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OMNICLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "omniclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OMNICLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
OMNICLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
