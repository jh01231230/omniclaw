...
This guide summarizes a sane workflow for working on the pi integration in OmniClaw.
...
- Trigger the agent directly:
  - `pnpm omniclaw agent --message "Hello" --thinking low`
...
State lives under the OmniClaw state directory. Default is `~/.omniclaw`. If `OMNICLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `omniclaw.json` for config
...
- https://docs.omniclaw.ai/testing
- https://docs.omniclaw.ai/start/getting-started
