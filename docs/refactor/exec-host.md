...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.omniclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.omniclaw/node.json` (node id + pairing token).
...
Path: `~/.omniclaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.omniclaw/exec-approvals.sock",
...
- Unix socket at `~/.omniclaw/exec-approvals.sock` (0600).
