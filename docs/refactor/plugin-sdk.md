...
- Publish as `omniclaw/plugin-sdk` (or export from core under `omniclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OmniClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OmniClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OmniClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OmniClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OmniClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OmniClawConfig): string;
  };
};
```
...
- Introduce `omniclaw/plugin-sdk`.
- Add `api.runtime` to `OmniClawPluginApi` with the surface above.
