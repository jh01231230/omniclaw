...
OmniClaw can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere OmniClaw can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, OmniClaw defaults
...
TTS config lives under `messages.tts` in `omniclaw.json`.
...
- If `provider` is **unset**, OmniClaw prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.omniclaw/settings/tts.json`, override with `OMNICLAW_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, OmniClaw retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, OmniClaw:
...
Discord note: `/tts` is a built-in Discord command, so OmniClaw registers
