---
summary: "OmniClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OmniClaw on a Raspberry Pi
  - Running OmniClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OmniClaw on Raspberry Pi

## Goal

Run a persistent, always-on OmniClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most OmniClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled omniclaw

# Enable if not
sudo systemctl enable omniclaw

# Start on boot
sudo systemctl start omniclaw
```
...
```bash
# Check logs
journalctl -u omniclaw --no-pager -n 100

# Common fix: rebuild
cd ~/omniclaw  # if using hackable install
npm run build
sudo systemctl restart omniclaw
```
