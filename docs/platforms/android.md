...
- You can run the CLI (`omniclaw`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.omniclaw/omniclaw.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `omniclaw.internal.`) on the gateway host and publish `_openclaw-gw._tcp` records.
...
1. Create `~/.omniclaw/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
omniclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__openclaw__/canvas/"}'
```
