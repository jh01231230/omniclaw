...
service via the external `omniclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`OMNICLAW_PROFILE`; legacy `com.omniclaw.*` is supported).
...
- Writes `~/.omniclaw/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.omniclaw/disable-launchagent`,
