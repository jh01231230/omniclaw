...
- **gateway host** → `omniclaw` process on the gateway machine
...
`~/.omniclaw/exec-approvals.json` directly.

CLI: `omniclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
