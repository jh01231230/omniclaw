---
summary: "Chrome extension: let OmniClaw drive your existing Chrome tab"
read_when:
...
The OmniClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate omniclaw-managed Chrome profile.
...
OmniClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OmniClaw release (npm package) as static files. There is no separate “build” step.

After upgrading OmniClaw:

- Re-run `omniclaw browser extension install` to refresh the installed files under your OmniClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

OmniClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want OmniClaw to control.
...
- `ON`: attached; OmniClaw can drive that tab.
...
Debugging: `omniclaw sandbox explain`
...
`omniclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `omniclaw browser extension install` first to copy the extension to a stable location under your OmniClaw state directory.
...
- **This is not isolated** like the dedicated omniclaw-managed profile.
