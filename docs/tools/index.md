---
summary: "Agent tool surface for OmniClaw (browser, canvas, nodes, message, cron) replacing legacy `omniclaw-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `omniclaw-*` skills
title: "Tools"
---

# Tools (OmniClaw)

OmniClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `omniclaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `omniclaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OmniClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`omniclaw node run`).
...
- Requires a Brave API key (recommended: `omniclaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OmniClaw-managed browser.
...
- Quick smoke: `omniclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `omniclaw gateway` restart in-place)
