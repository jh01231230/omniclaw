...
This document explains how OmniClaw manages sessions end-to-end:
...
OmniClaw is designed around a single **Gateway process** that owns session state.
...
OmniClaw persists sessions in two layers:
...
- Transcripts: `~/.omniclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

OmniClaw resolves these via `src/config/sessions.ts`.
...
OmniClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (OmniClaw consumes the events, but Pi decides when to compact).
...
OmniClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, OmniClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, OmniClaw leaves it alone.
...
- `omniclaw status` (CLI)
- `omniclaw sessions` / `sessions --json`
...
OmniClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- OmniClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, OmniClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
OmniClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but OmniClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `omniclaw status`.
