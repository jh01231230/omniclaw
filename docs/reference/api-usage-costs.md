...
OmniClaw features that can generate provider usage or paid API calls.
...
- `omniclaw status --usage` and `omniclaw channels list` show provider **usage windows**
...
OmniClaw can pick up credentials from:
...
`omniclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
