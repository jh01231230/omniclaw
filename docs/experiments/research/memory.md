...
read_when:
  - Designing workspace memory (~/.omniclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OmniClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.omniclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.omniclaw/workspace` as canonical human-readable memory.
...
```
~/.omniclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OmniClaw**, but keep a separable core library.

### Why integrate into OmniClaw?

- OmniClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `omniclaw memory recall "…" --k 25 --since 30d`
  - `omniclaw memory reflect --since 7d`
...
Pragmatic take for `~/.omniclaw/workspace`:
