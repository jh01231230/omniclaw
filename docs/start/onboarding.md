---
summary: "First-run onboarding flow for OmniClaw (macOS app)"
read_when:
...
- Writes credentials to `~/.omniclaw/credentials/oauth.json`
...
The app can install the global `omniclaw` CLI via npm/pnpm so terminal
...
On the first agent run, OmniClaw bootstraps a workspace (default `~/.omniclaw/workspace`):
...
```bash
omniclaw webhooks gmail setup --account you@gmail.com
```
...
- `~/.omniclaw/credentials/oauth.json`
