---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OmniClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OmniClaw cron scheduling for periodic checks, or version status checks on a machine running OmniClaw (laptop, workstation, Pi, VPS).
---

# OmniClaw Host Hardening

## Overview

Assess and harden the host running OmniClaw, then align it to a user-defined risk tolerance without breaking access. Use OmniClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OmniClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. OmniClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OmniClaw can access sensitive data.
...
### 2) Run OmniClaw security audits (read-only)

As part of the default read-only checks, run `omniclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `omniclaw security audit` (faster, non-probing)
2. `omniclaw security audit --json` (structured output)

Offer to apply OmniClaw safe defaults (numbered):

1. `omniclaw security audit --fix`

Be explicit that `--fix` only tightens OmniClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OmniClaw version/update status (read-only)

As part of the default read-only checks, run `omniclaw update status`.
...
- Credential hygiene notes (location of OmniClaw creds, prefer disk encryption)
...
- OmniClaw security audit (re-run)
...
After OmniClaw install or first hardening pass, run at least one baseline audit and version check:

- `omniclaw security audit`
- `omniclaw security audit --deep`
- `omniclaw update status`

Ongoing monitoring is recommended. Use the OmniClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `omniclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `omniclaw update status`

Also offer a periodic version check so the user can decide when to update (numbered):

1. `omniclaw update status` (preferred for source checkouts and channels)
2. `npm view omniclaw version` (published npm version)

## OmniClaw command accuracy

Use only supported commands and flags:

- `omniclaw security audit [--deep] [--fix] [--json]`
- `omniclaw status` / `omniclaw status --deep`
- `omniclaw health --json`
- `omniclaw update status`
- `omniclaw cron add|list|runs|run`

Do not invent CLI flags or imply OmniClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OmniClaw compaction:
