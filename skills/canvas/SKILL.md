...
Display HTML content on connected OmniClaw nodes (Mac app, iOS, Android).
...
```bash
cat ~/.omniclaw/omniclaw.json | jq '.gateway.bind'
```
...
1. Check server bind: `cat ~/.omniclaw/omniclaw.json | jq '.gateway.bind'`
...
Node is offline. Use `omniclaw nodes list` to find online nodes.
