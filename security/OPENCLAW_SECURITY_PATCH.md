# OmniClaw Security Patchset

This document details the known security risks in OmniClaw and the applied mitigations.
...
- `public_bind_guard` module refuses public bind without:
  - `OMNICLAW_ALLOW_PUBLIC_BIND=true` env var
  - `OMNICLAW_PUBLIC_BIND_IP_ALLOWLIST` configured (IP/CIDR; enforced for HTTP + WS)
...
- Remote skills installation disabled by default (`OMNICLAW_SKILLS_ALLOW_REMOTE_INSTALL=false`)
...
The following are **strictly prohibited** in OmniClaw deployments:
...
- [Runbook](./OMNICLAW_RUNBOOK.md) - Incident response procedures
