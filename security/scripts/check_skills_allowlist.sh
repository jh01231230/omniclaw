#!/usr/bin/env bash
# OmniClaw Security Check: Skills Allowlist Verification
...
SKILLS_DIR="$OMNICLAW_DIR/skills_local"
ALLOWLIST_FILE="$OMNICLAW_DIR/config/skills_allowlist.json"

echo "OmniClaw Security Check: Skills Allowlist"
echo "=========================================="
...
CONFIG_FILE="$OMNICLAW_DIR/config.yaml"
...
if [[ "${OMNICLAW_SKILLS_ALLOW_REMOTE_INSTALL:-false}" == "true" ]]; then
  echo -e "${RED}WARNING: OMNICLAW_SKILLS_ALLOW_REMOTE_INSTALL=true${NC}"
...
if [[ $EXIT_CODE -eq 0 ]]; then
  ...
  echo "  4. Set OMNICLAW_SKILLS_ALLOW_REMOTE_INSTALL=false"
fi
