#!/usr/bin/env bash
# OmniClaw Security Check: Public Bind Detection
...
echo "OmniClaw Security Check: Public Bind Detection"
echo "================================================"
...
CONFIG_FILES=(
  "$HOME/.omniclaw/config.yaml"
  "$HOME/.omniclaw/config.yml"
  "$HOME/.omniclaw/config.json"
)
...
if [[ "${OMNICLAW_GATEWAY_HOST:-}" =~ ^0\.0\.0\.0$ ]] || [[ "${OMNICLAW_GATEWAY_HOST:-}" == "::" ]]; then
  if [[ -z "${OMNICLAW_GATEWAY_TOKEN:-}" ]] && [[ -z "${OMNICLAW_GATEWAY_PASSWORD:-}" ]]; then
    echo -e "${RED}WARNING: OMNICLAW_GATEWAY_HOST is public but no auth token/password is set${NC}"
    EXIT_CODE=1
  fi
fi
...
if command -v ss &>/dev/null; then
  # Look for omniclaw gateway listening on 0.0.0.0
  PUBLIC_LISTENERS=$(ss -tlnp 2>/dev/null | grep -E '0\.0\.0\.0:18789|:::18789' | grep -i omniclaw || true)
  if [[ -n "$PUBLIC_LISTENERS" ]]; then
    echo -e "${RED}WARNING: OmniClaw gateway is listening on public interface:${NC}"
    echo "$PUBLIC_LISTENERS"
    EXIT_CODE=1
  fi
elif command -v netstat &>/dev/null; then
  PUBLIC_LISTENERS=$(netstat -tlnp 2>/dev/null | grep -E '0\.0\.0\.0:18789|:::18789' | grep -i omniclaw || true)
  if [[ -n "$PUBLIC_LISTENERS" ]]; then
    echo -e "${RED}WARNING: OmniClaw gateway is listening on public interface:${NC}"
    echo "$PUBLIC_LISTENERS"
    EXIT_CODE=1
...
if [[ $EXIT_CODE -eq 0 ]]; then
  ...
  echo "     - OMNICLAW_ALLOW_PUBLIC_BIND=true is set"
  echo "     - OMNICLAW_PUBLIC_BIND_IP_ALLOWLIST is configured"
  echo "     - Gateway TLS is enabled (gateway.tls.enabled=true)"
  echo "     - Gateway auth is configured (OMNICLAW_GATEWAY_TOKEN or OMNICLAW_GATEWAY_PASSWORD)"
fi
