...
echo -e "${BOLD}OmniClaw Security Smoketest${NC}"
echo "============================"
...
OMNICLAW_DIR="${OMNICLAW_DIR:-$HOME/.omniclaw}"
if [[ -f "$OMNICLAW_DIR/config.yaml" ]]; then
  PERMS=$(stat -c %a "$OMNICLAW_DIR/config.yaml" 2>/dev/null || stat -f %OLp "$OMNICLAW_DIR/config.yaml" 2>/dev/null || echo "unknown")
...
if [[ -d "$OMNICLAW_DIR" ]]; then
  PERMS=$(stat -c %a "$OMNICLAW_DIR" 2>/dev/null || stat -f %OLp "$OMNICLAW_DIR" 2>/dev/null || echo "unknown")
...
if pgrep -f "omniclaw.*gateway" >/dev/null 2>&1 || pgrep -f "omniclaw-gateway" >/dev/null 2>&1; then
