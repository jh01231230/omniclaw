# OmniClaw Security Runbook

Incident response procedures for OmniClaw security events.
...
1. **Stop the gateway**
   ...
   ```bash
   pkill -f omniclaw-gateway
   # Or via systemd: systemctl stop omniclaw-gateway
   ```
...
3. **Check for unauthorized access**
   ...
   ```bash
   # Review gateway logs
   tail -n 1000 /tmp/omniclaw/omniclaw-*.log | grep -E 'auth|connect|unauthorized'

   # Check for config changes
   diff ~/.omniclaw/config.yaml ~/.omniclaw/config.yaml.bak
   ```
...
1. **Disable the suspicious skill**
   ...
   omniclaw config set plugins.<skill_id>.enabled false

   # Or remove skill files entirely
   rm -rf ~/.omniclaw/skills_local/<skill_id>/
   ```
...
2. **Kill any running skill processes**
   ...
   pgrep -f 'omniclaw.*skill' | xargs kill -9
   ```
...
1. **Audit skill execution history**
   ...
   ```bash
   grep -r 'skill.*execute' /tmp/omniclaw/omniclaw-*.log
   ```
...
3. **Verify workspace integrity**
   ...
   find ~/.omniclaw -mtime -1 -type f

   # Check file hashes if baseline exists
   find ~/.omniclaw -type f -exec sha256sum {} \; | diff - baseline.txt
   ```
...
- Enable skills allowlist (`OMNICLAW_SKILLS_ALLOWLIST`)
...
1. **Review recent messages** for impersonated content
   ...
   grep -E 'impersonate|post_as|actor.*override' /tmp/omniclaw/omniclaw-*.log
   ```
...
grep 'SECURITY_EVENT.*auth.*fail' /tmp/omniclaw/*.log

# Rate limit triggers
grep 'SECURITY_EVENT.*rate.*limit' /tmp/omniclaw/*.log

# Blocked dangerous patterns
grep 'SECURITY_EVENT.*blocked' /tmp/omniclaw/*.log

# Anomaly detections
grep 'SECURITY_EVENT.*anomaly' /tmp/omniclaw/*.log
```
...
```bash
export OMNICLAW_SECURITY_WEBHOOK_URL="https://your-alerting-service.com/webhook"
```
...
- **Security Issues**: security@omniclaw.ai
- **Critical Incidents**: [escalation process]
- **Bug Reports**: https://github.com/omniclaw/omniclaw/issues
