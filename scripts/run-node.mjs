...
const compilerOverride = env.OMNICLAW_TS_COMPILER ?? env.CLAWDBOT_TS_COMPILER;
...
const logRunner = (message) => {
  if (env.OMNICLAW_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[omniclaw] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["omniclaw.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
