...
BASE_IMAGE="${BASE_IMAGE:-omniclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-omniclaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=omniclaw.sandbox=1)
NOTE
