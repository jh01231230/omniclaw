...
IMAGE_NAME="omniclaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/omniclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/omniclaw-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/omniclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/omniclaw-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/omniclaw-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.omniclaw/workspace"
    config_path="$HOME/.omniclaw/omniclaw.json"
    sessions_dir="$HOME/.omniclaw/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.omniclaw/omniclaw.json" <<'"'"'JSON'"'"'
{
