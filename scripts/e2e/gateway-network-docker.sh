...
IMAGE_NAME="omniclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="omniclaw-net-e2e-$$"
GW_NAME="omniclaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "OMNICLAW_GATEWAY_TOKEN=$TOKEN" \
	  -e "OMNICLAW_SKIP_CHANNELS=1" \
	  -e "OMNICLAW_SKIP_GMAIL_WATCHER=1" \
	  -e "OMNICLAW_SKIP_CRON=1" \
	  -e "OMNICLAW_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
