...
IMAGE_NAME="omniclaw-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/omniclaw-bin:$PATH"
  mkdir -p /tmp/omniclaw-bin

  cat > /tmp/omniclaw-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/omniclaw-bin/systemctl

  cat > /tmp/omniclaw-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/omniclaw-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/omniclaw"
  npm_entry="/tmp/npm-prefix/lib/node_modules/omniclaw/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/omniclaw.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/omniclaw-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/omniclaw-gateway.service"
    if [ ! -f "$unit_path" ]; then
