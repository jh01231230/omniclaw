#!/bin/bash
# Setup OmniClaw Auth Management System
...
echo "=== OmniClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'omniclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OmniClaw message: Send warning via OmniClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/omniclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/omniclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/omniclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now omniclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status omniclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
