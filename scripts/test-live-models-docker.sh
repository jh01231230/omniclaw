...
docker run --rm -t \
  ...
  -e OMNICLAW_LIVE_MODELS="${OMNICLAW_LIVE_MODELS:-${CLAWDBOT_LIVE_MODELS:-all}}" \
  -e OMNICLAW_LIVE_PROVIDERS="${OMNICLAW_LIVE_PROVIDERS:-${CLAWDBOT_LIVE_PROVIDERS:-}}" \
  -e OMNICLAW_LIVE_MODEL_TIMEOUT_MS="${OMNICLAW_LIVE_MODEL_TIMEOUT_MS:-${CLAWDBOT_LIVE_MODEL_TIMEOUT_MS:-}}" \
  -e OMNICLAW_LIVE_REQUIRE_PROFILE_KEYS="${OMNICLAW_LIVE_REQUIRE_PROFILE_KEYS:-${CLAWDBOT_LIVE_REQUIRE_PROFILE_KEYS:-}}" \
