...
function trySetupCompletion(repoRoot) {
  ...
  if (process.env.CI || process.env.OMNICLAW_SKIP_COMPLETION_SETUP) {
    return;
  }

  const binPath = path.join(repoRoot, "omniclaw.mjs");
  ...
  try {
    // Run with OMNICLAW_SKIP_POSTINSTALL to avoid any weird recursion,
    // though distinct from this script.
    spawnSync(process.execPath, [binPath, "completion", "--install", "--yes"], {
      cwd: repoRoot,
      stdio: "inherit",
      env: { ...process.env, OMNICLAW_SKIP_POSTINSTALL: "1" },
    });
  } catch {
...
try {
  const skip =
    process.env.OMNICLAW_SKIP_POSTINSTALL === "1" ||
