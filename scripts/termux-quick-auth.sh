...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/omniclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OmniClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/omniclaw/scripts/mobile-reauth.sh'" --id omniclaw-auth
        ;;
    *)
