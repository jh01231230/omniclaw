...
INSTALL_URL="${OMNICLAW_INSTALL_URL:-https://omniclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OMNICLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OMNICLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="omniclaw"
PACKAGE_NAME="${OMNICLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OMNICLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OMNICLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OMNICLAW_INSTALL_LATEST_OUT:-}"
fi
