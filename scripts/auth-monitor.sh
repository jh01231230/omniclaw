...
#
# Suggested cron: */30 * * * * /home/admin/omniclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OmniClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., omniclaw-alerts)
...
STATE_FILE="$HOME/.omniclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OmniClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use omniclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OmniClaw to $NOTIFY_PHONE..."
            omniclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OmniClaw is down. Run: ssh l36 '~/omniclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
