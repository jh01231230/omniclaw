...
ZIP=${1:?"Usage: $0 OmniClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/omniclaw/omniclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^OmniClaw-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/omniclaw/omniclaw/releases/download/v${VERSION}/"}
