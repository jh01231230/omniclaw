...
import Foundation
import Testing
@testable import OmniClawChatUI
...
private final class TestChatTransport: @unchecked Sendable, OmniClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OmniClawChatHistoryPayload]
    private let sessionsResponses: [OmniClawChatSessionsListResponse]

    private let stream: AsyncStream<OmniClawChatTransportEvent>
    private let continuation: AsyncStream<OmniClawChatTransportEvent>.Continuation

    init(
        historyResponses: [OmniClawChatHistoryPayload],
        sessionsResponses: [OmniClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OmniClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OmniClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OmniClawChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [OmniClawChatAttachmentPayload]) async throws -> OmniClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return OmniClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OmniClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OmniClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OmniClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = OmniClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = OmniClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { OmniClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = OmniClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = OmniClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = OmniClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { OmniClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
