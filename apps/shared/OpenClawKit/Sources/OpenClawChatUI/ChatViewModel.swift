...
private let chatUILogger = Logger(subsystem: "ai.omniclaw", category: "OmniClawChatUI")

@MainActor
@Observable
public final class OmniClawChatViewModel {
    public private(set) var messages: [OmniClawChatMessage] = []
    ...
    public var attachments: [OmniClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OmniClawChatPendingToolCall] = []
    public private(set) var sessions: [OmniClawChatSessionEntry] = []
    private let transport: any OmniClawChatTransport
    ...
    private var pendingToolCallsById: [String: OmniClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OmniClawChatTransport) {
    ...
    public var sessionChoices: [OmniClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [OmniClawChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [OmniClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OmniClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OmniClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OmniClawChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [OmniClawChatMessage]) -> [OmniClawChatMessage] {
        var result: [OmniClawChatMessage] = []
    ...
    private static func dedupeKey(for message: OmniClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OmniClawChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> OmniClawChatAttachmentPayload in
            OmniClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OmniClawChatSessionEntry {
        OmniClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OmniClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OmniClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OmniClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OmniClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OmniClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
