...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OmniClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OmniClawProtocol.AnyCodable] = [
            "presence": .init([OmniClawProtocol.AnyCodable(entry)]),
        ]
        let payload = OmniClawProtocol.AnyCodable(payloadMap)
