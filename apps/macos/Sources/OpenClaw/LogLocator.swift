...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OMNICLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/omniclaw")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("omniclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("omniclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/omniclaw/ (rolling or stdout), or nil if none exist.
