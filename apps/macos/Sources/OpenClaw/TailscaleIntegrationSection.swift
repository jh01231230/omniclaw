...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.omniclaw/omniclaw.json. Prefer OMNICLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.omniclaw/omniclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.omniclaw/omniclaw.json. Restart the gateway to apply."
        }
