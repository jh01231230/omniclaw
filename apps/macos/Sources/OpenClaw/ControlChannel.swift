...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OmniClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.omniclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OmniClawKit.AnyCodable]()) {
                $0[$1.key] = OmniClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OmniClawProtocol.AnyCodable?) -> [String: OmniClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OmniClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OmniClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OmniClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("omniclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("omniclaw.control.agent")
}
