...
enum DebugActions {
    private static let verboseDefaultsKey = "omniclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "omniclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OmniClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OmniClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OmniClawPaths.configURL
        guard
