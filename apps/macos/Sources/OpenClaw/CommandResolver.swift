...
enum CommandResolver {
    private static let projectRootDefaultsKey = "omniclaw.gatewayProjectRootPath"
    private static let helperName = "omniclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let openclawEntry = root.appendingPathComponent("omniclaw.mjs").path
        if FileManager().isReadableFile(atPath: openclawEntry) { return openclawEntry }
        let binEntry = root.appendingPathComponent("bin/omniclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/omniclaw")
    ...
    private static func openclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".omniclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func projectOmniClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("omniclaw.mjs").path,
            root.appendingPathComponent("bin/omniclaw.js").path,
        ]
    ...
    static func hasAnyOmniClawInvoker(searchPaths: [String]? = nil) -> Bool {
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let openclawPath = self.projectOmniClawExecutable(projectRoot: root) {
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "omniclaw", subcommand] + extraArgs
            }
            ...
            let missingEntry = """
            omniclaw entrypoint missing (looked for dist/index.js or omniclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real omniclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/omniclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v omniclaw >/dev/null 2>&1; then
          CLI="$(command -v omniclaw)"
          omniclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/omniclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/omniclaw.mjs"
            node "$PRJ/omniclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/omniclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/omniclaw.js"
            node "$PRJ/bin/omniclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent omniclaw"
          pnpm --silent omniclaw \(quotedArgs);
        else
          echo "omniclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OmniClawConfigFile.loadDict()
