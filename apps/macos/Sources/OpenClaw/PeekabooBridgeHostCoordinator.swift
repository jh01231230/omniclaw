...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.omniclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OmniClawPeekabooBridgeServices?
    private static var openclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("OmniClaw", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = OmniClawPeekabooBridgeServices()
...
@MainActor
private final class OmniClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.omniclaw.peekaboo")
