...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.omniclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OmniClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OmniClawSessionPreviewEntry,
    ...
    private static func cache(payload: OmniClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OmniClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OmniClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OmniClawKit.AnyCodable]) -> [OmniClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OmniClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OmniClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OmniClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OmniClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OmniClawChatMessage) -> String? {
