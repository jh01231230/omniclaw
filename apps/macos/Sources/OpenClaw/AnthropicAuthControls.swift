...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: OmniClawOAuthStore.AnthropicOAuthStatus = OmniClawOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: OmniClawOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = OmniClawOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = OmniClawOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try OmniClawOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. OmniClaw can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: OmniClawOAuthStore.AnthropicOAuthStatus,
