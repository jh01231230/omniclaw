...
enum OmniClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = OmniClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OmniClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OmniClawLogging.bootstrapIfNeeded()
        let label = OmniClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OmniClawOSLogHandler: LogHandler {
...
struct OmniClawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OmniClawLogging.parseLabel(self.label)
