...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.omniclaw", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["OMNICLAW_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = OmniClawConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
        ...
        case let .success(runtime):
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "omniclaw CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["OMNICLAW_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "omniclaw@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "omniclaw@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "omniclaw@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "omniclaw@\(target)"])
            }

        statusHandler("Installing omniclaw@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed omniclaw@\(target)")
        } else {
