...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = OmniClawCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = OmniClawCanvasA2UIAction.compactJSON(userAction["context"])
        ...
        let message = OmniClawCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = OmniClawCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: OmniClawLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for omniclaw:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case OmniClawLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case OmniClawCanvasCommand.present.rawValue,
                 OmniClawCanvasCommand.hide.rawValue,
                 OmniClawCanvasCommand.navigate.rawValue,
                 OmniClawCanvasCommand.evalJS.rawValue,
                 OmniClawCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case OmniClawCanvasA2UICommand.reset.rawValue,
                 OmniClawCanvasA2UICommand.push.rawValue,
                 OmniClawCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case OmniClawCameraCommand.list.rawValue,
                 OmniClawCameraCommand.snap.rawValue,
                 OmniClawCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case OmniClawScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: OmniClawNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: OmniClawNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(OmniClawLocationGetParams.self, from: req.paramsJSON)) ??
            OmniClawLocationGetParams()
        ...
        let payload = OmniClawLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OmniClawCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(OmniClawCanvasPresentParams.self, from: req.paramsJSON)) ??
                OmniClawCanvasPresentParams()
        ...
        case OmniClawCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case OmniClawCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(OmniClawCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case OmniClawCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(OmniClawCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case OmniClawCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(OmniClawCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case OmniClawCanvasA2UICommand.reset.rawValue:
            ...
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case OmniClawCanvasA2UICommand.push.rawValue, OmniClawCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == OmniClawCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(OmniClawCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try OmniClawCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(OmniClawCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try OmniClawCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.openclawA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case OmniClawCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case OmniClawCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(OmniClawCameraSnapParams.self, from: req.paramsJSON)) ??
                OmniClawCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case OmniClawCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(OmniClawCameraClipParams.self, from: req.paramsJSON)) ??
                OmniClawCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(OmniClawScreenRecordParams.self, from: req.paramsJSON)) ??
            OmniClawScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> OmniClawLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return OmniClawLocationMode(rawValue: raw) ?? .off
    }
