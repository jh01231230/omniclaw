...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [OmniClawCapability.canvas.rawValue, OmniClawCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(OmniClawCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(OmniClawCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = OmniClawLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(OmniClawCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            OmniClawSystemCommand.notify.rawValue,
            OmniClawSystemCommand.which.rawValue,
            OmniClawSystemCommand.run.rawValue,
            OmniClawSystemCommand.execApprovalsGet.rawValue,
            OmniClawSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(OmniClawCapability.camera.rawValue) {
            commands.append(OmniClawCameraCommand.list.rawValue)
            commands.append(OmniClawCameraCommand.snap.rawValue)
            commands.append(OmniClawCameraCommand.clip.rawValue)
        }
        if caps.contains(OmniClawCapability.location.rawValue) {
            commands.append(OmniClawLocationCommand.get.rawValue)
        }
