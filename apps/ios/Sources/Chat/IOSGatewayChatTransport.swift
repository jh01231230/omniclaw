...
struct IOSGatewayChatTransport: OmniClawChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OmniClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OmniClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OmniClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OmniClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OmniClawChatAttachmentPayload]) async throws -> OmniClawChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OmniClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(OmniClawChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(OmniClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OmniClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OmniClawChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OmniClawAgentEventPayload.self)
                        {
